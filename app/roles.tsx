...
export default function RolesScreen() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (dto: { name: string }) => {
      if (editing) return rolesService.update(editing.id, dto);
      return rolesService.create(dto);
    },
    onSuccess: () => {
      showToast(editing ? 'Role berhasil diperbarui' : 'Role berhasil ditambahkan', 'success');
      closeForm();
  ...
  const handleDelete = (r: Role) => {
    Alert.alert('Hapus Role', `Yakin hapus "${r.name}"?`, [
      { text: 'Batal', style: 'cancel' },
      { text: 'Hapus', style: 'destructive', onPress: () => deleteMutation.mutate(r.id) },
    ]);
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Role & Izin</Text>
      </View>

      <Button title="+ Tambah Role" size="sm" onPress={openCreate} style={styles.addBtn} />
      ...
      <FlatList
        ...
        ListEmptyComponent={
          isLoading ? (
            ...
            <Text style={styles.empty}>Tidak ada role</Text>
          )
        }
      />

      {showForm && (
        <Modal visible onClose={closeForm} title={editing ? 'Edit Role' : 'Tambah Role'}>
